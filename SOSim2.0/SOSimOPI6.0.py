...
Copyright (C) 2010 M. Angelica Echavarria Gregory

This file is part of SOSim - Sunken Oil Simulator.
...
by a grant from NOAA/UNH Coastal Response Research Center.
NOAA Grant Number: NA04NOS4190063.  Project Number: 08-088
 
It is also a Deliverable of PhD Thesis and dissertation for PhD degree of M. Angelica Echavarria Gregory
University of Miami,
Jan 2007 - August 2010.

For development support contact @umiami.edu
...
import SOSimgui_Copy2 as SOSimgui
from ui_Options import Ui_MyDialog
#import SOSimCore
import submerged222 as submerged 
import sunken3 as sunken
#import date
...
class SOSimMainWindow(QMainWindow, SOSimgui.Ui_SOSimMainWindow):
    ...
    def __init__(self, parent=None): 
        ...
        #self.connect(self.ManSelectAreaButton, SIGNAL("clicked()"), self.zoomInArea)
        ...
        #self.mapToolbar = QToolBar( "Map", self )
        ...
        # Add the zoom-in action to the Manually Select Area Button (works with zoom-to-polygon):
        #self.ManSelectAreaButton.addAction(self.actionZoomIn)
        ...
        #self.connect(self.UserDefinedScaleRadioButton, SIGNAL("toggled(bool)"), self.ScaleFrame, SLOT("setVisible(bool)"))
        # Nodes options:
        ...
        #self.NextTimeButton.setEnabled(False)
        #self.PreviousTimeButton.setEnabled(False)
        self.RecalcButton.setEnabled(False)
        # self.CalcTimeNumberSpinBox.setEnabled(False)
        # self.CampaignNumberSpinBox.setEnabled(False)
...
#___________________________________________________________________________________________________________________
        ...
        # Draw north arrow:
        #self.drawNorth(-180.0, -90.0, 180.0, 90.0)
...
#__________________________________________________________________________________________________________________________
    ...
    def addRasterImage(self):
        ...
        fileName = 'Desktop'
        #file = QFileDialog.getOpenFileName(self, "Import Raster Image",".","PNG(*.PNG)")
        ...
        if file:
            ...
            # layer = QgsRasterLayer(fileName,file)
    ...
    def addRasterImage1(self):
        ...
        if file:
            ...
            self.layers.insert(0, QgsMapCanvasLayer(layer))
            #self.layers[0] = QgsMapCanvasLayer(layer)
...
#        lat0 = self.LatSpillEdit.text()
        ...
        # self.ourinformation['f2'] = 'n'
        self.showSpillSiteCanvas(4)
        self.drawSpill0()
#        self.retardationDueOilType()
        self.TitleEdit.setText("MODEL SET UP FOR SPILL %s" % self.spillName().upper())
        #if not self.SpillDateEdit.dateChanged():
        #    QMessageBox.warning(self, "SOSim - Empty field", "Please define a spill date from the calendar before continuing")           
    @pyqtSignature("")
...
#        self.retardationDueOilType()
    ...
    def drawSpill0(self):
        if self.submerged.isChecked():
            x = self.lon0    
            y = self.lat0
            x=float(x) - 0.01875  # the error 
            y=float(y) + 0.02112  # the error 
            print x
            print y
        ...
        if self.sunken.isChecked():
            x = self.lon0    
            y = self.lat0
            x=float(x) - 0.04875  # the error 
            y=float(y) + 0.06612  # the error 
            print x
            print y
            if len( self.markers ) > 0:
                self.canvas.scene().removeItem( self.markers[ 0 ] )
    ...
    def on_UploadCampaignButton_clicked(self):
        if not (self.SpillE.isChecked() or self.SpillW.isChecked()) or not(self.SpillN.isChecked() or self.SpillS.isChecked()):
            ...
            try:
                ...
                print(DL)
                LonWGS = map(float,DL[1])
                print "LonWGS: %s" % LonWGS
                LatWGS = map(float,DL[2])
                print "LatWGS: %s" % LatWGS
                DLcon = map(float,DL[3]) 
                #print DLcon
                LatLongUTM = []
                i = 0
                while i < len(LonWGS):
                ...
                for s in DLcon:
                    if s == 0.0:
                        ...
                        conValue = s#/100.0    # change here when for PPL concentration
                ...
                if more == QMessageBox.Yes:
                    ...
                    self.CampaignNumberSpinBox.setValue(len(self.ourinformation['CampaignButton']))
                    # self.CampaignNumberSpinBox.selectAll()
                ...
                if more == QMessageBox.No:
                    ...
                    #print "DLx BEFORE ADJUSTING ZONES: %s" % self.DLx
	                ...
	                # print "Average of zones: %.4f" % average
	                if average is not int:
	                    ...
	                    for bad_zone in DLzoneJoin:
	                        if bad_zone != good_zone:
	                            ...
	                            # print "checking new zone: %.2f" % (LatLongConverter(auxiliar_point_longitude, auxiliar_point_latitude))[2]
	                            # print "vs. good zone: %i" % good_zone
    ...
    def on_RemoveSubmergedButton_clicked(self):
        ...
        if self.DLcon[index] != 0.0:
            ...
            if more == QMessageBox.Yes:
                ...
                #print self.DLy
                #print self.DLcon
                # self.CampaignNumberSpinBox.setValue(1)
                # self.CampaignNumberSpinBox.selectAll()
                # self.CampaignNumberSpinBox.setFocus()
...
#__________________________________________________________________________________________________________________________
    ...
    def SetSampTime(self):  
        ...
        if stlist[0] < 0.0:
            ...
            #if st > daysinmonth:
            #    QMessageBox.message(self, "SOSim - Long Term Sampling Campaign", "The sampling campaign is considered log term by SOSim. If this is the first and only sampling campaign, output predictions will not be conclusive. This campaign should be accompanied by other campaigns performed in different likely areas for the calibration to be accurate.")
            print st
        return st
    ...
    def SetDurationTime(self):  
        ...
        print "Verify resta ok to check st final:", drlist
        daysinmonth = (calendar.monthrange(int(TimeZero[0]),int(TimeZero[1])))[1]
        a=1
        daysinyear = 0
        while a <= 12:
...
#__________________________________________________________________________________________________________________________
    ...
    def on_SureButton_clicked(self):
        ...
        print float(x)-lon_s,float(y)-lat_s,float(x)+lon_s,float(y)+lat_s
        mc = self.canvas
        mc.setExtent(rect)
        mc.refresh()
        mglobal = self.globalcanvas
    ...
    def checkResolutionConflict(self):
        print self.x_max
        print self.x_min
        #if (self.x_max - self.x_min)/self.popDialog.user_nodes_x >= self.popDialog.defaultResolution or (self.y_max - self.y_min)/self.popDialog.user_nodes_y >= self.popDialog.defaultResolution: # maximum length is 30 nautical miles, or 55.56 km in each direction (resolution reasons).
        #    QMessageBox.information(self, "SOSim - Re-set Modeling Area", "The selected area is too large or the amount of nodes too little for the resolution to be good. Please select a smaller area or more nodes to model.")        
    ...
    def on_DefaultNodesRadioButton_clicked(self):
        ...
        areaselection = QMessageBox.question(self, "SOSim - Model Node Setting",
                                 "Please confirm that the current grid resolution is correct, the default node is 25",
                                 QMessageBox.Yes|QMessageBox.No)
        areaselection
    ...
    def Getinformation(self):
        ...
        days = int(spilldate.day())
        years = int(spilldate.year())
        # s2 = str(years) + "-" + str(months) + "-" + str(days)
        ...
        # self.ourinformation['PredictTime'] = s2 + " " + s1
        ...
        if self.UserDefinedScaleRadioButton.isChecked():
            ...
            #     self.ourinformation['lonscale'] = 0.05
            #     self.ourinformation['latscale'] = 0.075
        ...
        if self.DefaultParameterButton_17.isChecked():
            ...
            self.ourinformation["level"] = 0.01 
...
#__________________________________________________________________________________________________________________________
    ...
    def on_OSCARButton_clicked(self):
        ...
        areaselection = QMessageBox.question(self, "SOSim - Hydrodynamic Model Selection",
                                 "Please confirm that you have choosed to upload OSCAR model output",
                                 QMessageBox.Yes|QMessageBox.No)
        areaselection
        if areaselection == QMessageBox.Yes:
    ...
    def on_GNOMEButton_clicked(self):
        """Gets x_nodes and y_nodes input by the user and generates global variable value"""
        areaselection = QMessageBox.question(self, "SOSim - Hydrodynamic Model Selection",
                                 "Please confirm that you have choosed to upload GNOME model output",
                                 QMessageBox.Yes|QMessageBox.No)
        areaselection
        if areaselection == QMessageBox.Yes:
...
########################################################
    ...
    def on_DecimalButton_clicked(self):
        ...
        areaselection = QMessageBox.question(self, "SOSim - Bathymetry Upload",
                                 "Please confirm that you have choosed to upload Decimal type",
                                 QMessageBox.Yes|QMessageBox.No)
        areaselection
        if areaselection == QMessageBox.Yes:
        	print "GNOME model has been selected"
        if areaselection == QMessageBox.No:
            QMessageBox.information(self, "SOSim - Bathymetry Upload", "Please choose the bathymetry type")
    ...
    def on_UTMButton_clicked(self):
        """Sets x_nodes and y_nodes global variables to the default set by user in the options menu, or 25 if unchanged"""
        areaselection = QMessageBox.question(self, "SOSim - Bathymetry Upload",
                                 "Please confirm you choose to upload the UTM file",
                                 QMessageBox.Yes|QMessageBox.No)
        areaselection
        if areaselection == QMessageBox.Yes:
        	print "Other model has been selected"
    ...
    def on_NodataButton_clicked(self):
        """Gets x_nodes and y_nodes input by the user and generates global variable value"""
        areaselection = QMessageBox.question(self, "SOSim - Bathymetry Upload",
                                 "Please confirm that no bathymetry upload",
                                 QMessageBox.Yes|QMessageBox.No)
        areaselection
        if areaselection == QMessageBox.Yes:
        	self.UploadSubmergedHydroButton.setEnabled(False)                             
...
#__________________________________________________________________________________________________________________________
    # Methods to generate stopping time, or a warning to the user about after what time the model results are no longer valid (after bouncing).



    # def pointTouches(self, xclicks, yclicks, newX, newY):
    #     """Point is within the coastal square-zone"""
    #     matrixTrue = []
    #     seg = 0
    #     while seg < 4: # need to be change later
    #         subseg = 0
    #         while subseg < 5:
    #             if ((min(xclicks[seg], xclicks[seg+1])+((subseg*(max(xclicks[seg], xclicks[seg+1])-min(xclicks[seg], xclicks[seg+1])))/4.0)) <= newX <= (min(xclicks[seg], xclicks[seg+1])+(((subseg+1)*(max(xclicks[seg], xclicks[seg+1])-min(xclicks[seg], xclicks[seg+1])))/4.0)))\
    #                and ((min(yclicks[seg], yclicks[seg+1])+((subseg*(max(yclicks[seg], yclicks[seg+1])-min(yclicks[seg], yclicks[seg+1])))/4.0)) <= newY <= (min(yclicks[seg], yclicks[seg+1])+(((subseg+1)*(max(yclicks[seg], yclicks[seg+1])-min(yclicks[seg], yclicks[seg+1])))/4.0))):
    #                 matrixTrue.append(True)
    #                 subseg += 1
    #             else:
    #                 matrixTrue.append(False)
    #                 subseg += 1
    #         seg += 1
    #     if True in matrixTrue:
    #         return True
    #     else:
    #         return False
    ...
    #         B = [max(self.DLcon[vld]) for vld in xrange(len(self.st))]
    #         C = [self.DLcon[vld].index(max(self.DLcon[vld])) for vld in xrange(len(self.st))]
    #         x0news = [(self.DLx[vld][C[vld]])*B[vld] for vld in xrange(len(self.st))]
    #         y0news = [(self.DLy[vld][C[vld]])*B[vld] for vld in xrange(len(self.st))]
    ...
    #         L = open(PlayPath + "/my_DLx.txt", "r") 
    #         DLx = pickle.load(L)
    #         L.close()

    #         M = open(PlayPath + "/my_DLx.txt", "r") 
    #         DLy = pickle.load(M)
    #         M.close()

    #         N = open(PlayPath + "/my_DLcon.txt", "r") 
    #         DLcon = pickle.load(N)
    #         N.close()
            
    #         B = [max(DLcon[vld]) for vld in xrange(len(allST))]
    #         C = [DLcon[vld].index(max(DLcon[vld])) for vld in xrange(len(allST))]
    #         x0news = [(DLx[vld][C[vld]])*B[vld] for vld in xrange(len(allST))]
    #         y0news = [(DLy[vld][C[vld]])*B[vld] for vld in xrange(len(allST))]
    #         tnews = [allST[vld]*B[vld] for vld in xrange(len(self.st))]

            
    #     AveDLx = sum(x0news)/sum(B)
    #     AveDLy = sum(y0news)/sum(B)
    #     Avet = (sum(tnews)/sum(B))-self.retardation
    #     AveVelx = (AveDLx - self.sunkx0)/Avet # sign gives the direction.
    #     AveVely = (AveDLy - self.sunky0)/Avet
    #     slope = (AveDLy - self.sunky0)/(AveDLx - self.sunkx0)
    #     newX = AveDLx
    #     newY = AveDLy
    #     print "newX, newY:", newX, newY
        
    #     while self.pointTouches(self.xclicks, self.yclicks, newX, newY) is not True:
    #         if (AveVelx > 0.0) and (AveVely > 0.0):
    #             newX += 1.0/10.0            # calculate if within coastal environment every 100 meters (1km/10)
    #             newY += abs(slope/10.0)
    #         if (AveVelx > 0.0) and (AveVely < 0.0):
    #             newX += 1.0/10.0
    #             newY -= abs(slope/10.0)
    ...
    #     L = sqrt(((newX - self.sunkx0)**2.0) + ((newY - self.sunky0)**2.0))
    #     vel = sqrt(((AveVelx)**2.0) + ((AveVely)**2.0))
    #     stopT = (L/abs(vel))*1.2 # safety factor of 1.2
    #     print "stopT: %s" % stopT
    #     return stopT
...
#__________________________________________________________________________________________________________________________
    ...
    def SetCalcTime(self):
        ...
        print "DateCalc in array:", TimeCalc
        TimeZero = numpy.array(self.SetTimeZero())
        print "TimeZeroAgain:", TimeZero
        TClist = TimeCalc - TimeZero
        print "TClist: should be with negatives:", TClist
        ...
        if TClist[0] < 0.0:
            ...
            if TC > 90:
#            if TC > daysinmonth:
                QMessageBox.warning(self, "SOSim - Long Term Date", "The prediction time is considered log term by SOSim. Check that sampling campaigns which dates are close to the requested prediction time are loaded to the model for likelihood update and to avoid inaccuracy")
        print "TC:", TC
        return TC
    ...
    def on_AddCalcTimeButton_clicked(self):
        ...
        print len(self.ourinformation['OurTime'])

        self.CalcTimeNumberSpinBox.setValue(len(self.ourinformation['OurTime']))
        #timenumber = unicode(self.CalcTimeNumberSpinBox.value())
        #index = int(timenumber) - 1
        TC = self.SetCalcTime()
        ...
        print "t:", self.t
        enable = len(self.t) > 1
        # self.NextTimeButton.setEnabled(enable)
        # self.PreviousTimeButton.setEnabled(enable)
...
#__________________________________________________________________________________________________________________________
    ...
    def on_RunButton_clicked(self):
        ...
        if self.submerged.isChecked():
            ...
            for i in range(len(self.ourinformation['OurTime'])):
                ...
                # number = 0
                # # myfile1 = myfile1_list[j]
                # for ii in  range(len(myfile1)-1):
                #     if(myfile1[ii]=='/' and myfile1[ii+1]!='/'):
                #         number = ii
                # self.MyLegendLoad(myfile1,myfile1[ii+1:])
        # elif self.ourinformation['flag'] == 0:
        elif  self.sunken.isChecked():
            for i in range(len(self.ourinformation['OurTime'])):
                self.progressBar.setValue(0)
                print self.ourinformation['OurTime']                
                self.ourinformation['PredictTime'] = self.ourinformation['OurTime'][i]
                myfile,myfile1 = sunken.sunken_main(self.ourinformation,self.progressBar)
                myfile_list.append(myfile)
                myfile1_list.append(myfile1)
                print "This is a file list spill"
                print myfile_list
                print myfile1_list
        ...
        # InsPrelim = SOSimCore.Preliminars()for j in range(len(myfile1_list)):
        ...
        # argus = args
        # argus.append(self.xclicks)
        # argus.append(self.yclicks)for j in range(len(myfile1_list)):
        # #     number = 0
        # #     myfile1 = myfile1_list[j]
        # #     for i in  range(len(myfile1)-1):
        # #         if(myfile1[i]=='/' and myfile1[i+1]!='/'):
        ...
        # self.x0y0DueSinkingRetardation()
        # InsLFClass.calculateLV(maxN, self.retardation, self.sunkx0, self.sunky0, self.openOcean)
        ...
        # # for j in range(len(myfile1_list)):
        # #     number = 0
        # #     myfile1 = myfile1_list[j]
        # #     for i in  range(len(myfile1)-1):
        # #         if(myfile1[i]=='/' and myfile1[i+1]!='/'):
        # #             number = i
        # #     self.MyLegendLoad(myfile1,myfile1[i+1:])
        # InsModelClass = SOSimCore.Model(spillname, self.boundSefor j in range(len(myfile1_list)):
        # #     number = 0
        # #     myfile1 = myfile1_list[j]
        # #     for i in  range(len(myfile1)-1):
        # #         if(myfile1[i]=='/' and myfile1[i+1]!='/'):
        # #             number = i
        # #     self.MyLegendLoad(myfile1,myfile1[i+1:])gments, GammaPossible, self.x0, self.y0, \
        #                                 self.sx0, self.sy0, selfor j in range(len(myfile1_list)):
        # #     number = 0
        # #     myfile1 = myfile1_list[j]
        # #     for i in  range(len(myfile1)-1):
        # #         if(myfile1[i]=='/' and myfile1[i+1]!='/'):
        # #             number = i
        # #     self.MyLegendLoad(myfile1,myfile1[i+1:])f.t, self.user_nodes_x, \
        #                                 self.user_nodes_y, selffor j in range(len(myfile1_list)):
        # #     number = 0
        # #     myfile1 = myfile1_list[j]
        # #     for i in  range(len(myfile1)-1):
        # #         if(myfile1[i]=='/' and myfile1[i+1]!='/'):
        # #             number = i
        # #     self.MyLegendLoad(myfile1,myfile1[i+1:]).x_min, self.x_max, self.y_min, \
        #                                 self.y_max, self.lon0, self.lat0, *argus)

        # print "ModelClass instance created %s" % time.asctime()
        # self.progressBar.setValue(40)
        
        # InsModelClass.convertPairs()
       
        # x = InsModelClass.request_x()
        # y = InsModelClass.request_y()
        
        # self.progressBar.setValue(55)
        
        # X = InsModelClass.X_plot()
        # Y = InsModelClass.Y_plot()
        
        # print "Calculations of the Predictive Gaussian in time started %s" % time.asctime()
        ...
        # InsModelClass.calculateCG(X, Y, x, y, self.t, x0new, y0new, self.admST, self.openOcean)
        # InsModelClass.mapping(spillname, X, Y, self.admST, self.t)

        # print "Finished %s" % time.asctime()
        # self.progressBar.setValue(80)
        # print "Started %s" % T1
                
        # print "model calculated and figures auto-saved"
        
        # self.emit(SIGNAL("delete_polyline"))

        
        # self.MyRasterLoad(PlayPath + "/Spill %s predicted at time %.1f days.png" % (spillname, self.t[0]+ self.admST),\
        #                   "Spill %s predicted at time %.1f days" % (spillname, self.t[0]+ self.admST))
        ...
        # self.CurrentTimeEdit.setText("%.3f days" % (self.t[0]+ self.admST))
       
        # self.valid = valid
        # self.args = args
        # self.GammaPossible = GammaPossible
        
        # self.canvas.saveAsImage(ResultsPath + "/Spill %s predicted at time %.2f days.png" % (spillname, self.t[0]+ self.admST))
        # shutil.copy(SOSimPath + "/Data/Legend.png", ResultsPath + "/Legend.png")
        # print "Starting run time: %s" % T1
        # print "Finish run time: %s" % time.asctime()
        # self.progressBar.setValue(100)
        # self.RecalcButton.setEnabled(True)
        # QMessageBox.information(self, "SOSim - Predictive Maps Ready", "The output maps for the requested prediction time(s) are now on the Results folder of your SOSim directory (C:\Program Files\SOSim\Results).")

        # self.Getinformation()
        # # self.ourinformation['submerged'] = 1
        # print self.ourinformation
        # submerged.submerged_main(self.ourinformation)
    @pyqtSignature("")
    def on_RecalcButton_clicked(self):
        ...
        if self.st != []:
            args = self.args
            GammaPossible = self.GammaPossible

        else:
            InsPrelim = SOSimCore.Preliminars()
            args = InsPrelim.doAll(self.Dxmin, self.Dymin, self.Dxmax, self.Dymax, self.vxmin, self.vymin, self.vxmax, self.vymax, self.romin, self.romax)
            GammaPossible = InsPrelim.methodGammaPossible()
        ...
        InsModelClass = SOSimCore.Model(spillname, self.boundSegments, GammaPossible, self.x0, self.y0, \
                                        self.sx0, self.sy0, self.t, self.user_nodes_x, \
                                        self.user_nodes_y, self.x_min, self.x_max, self.y_min, \
                                        self.y_max, self.lon0, self.lat0, *argus)


        print "ModelClass instance for recalculation created %s" % time.asctime()
        self.progressBar.setValue(40)
        
        nodes_x = InsModelClass.nodes_x()
        nodes_y = InsModelClass.nodes_y()
        ...
        if self.st != []:
            ...
            allDLcon = pickle.load(L)
            L.close()
            M = open(PlayPath + "/my_DLx.txt", "r") #Saved as calibration and then saved to path2 to be readily available to "open" command.
            allDLx = pickle.load(M)
            M.close()
            N = open(PlayPath + "/my_DLy.txt", "r") #Saved as calibration and then saved to path2 to be readily available to "open" command.
            allDLy = pickle.load(N)
            N.close()
            B = [max(allDLcon[vld]) for vld in xrange(len(allST))]
            C = [allDLcon[vld].index(max(allDLcon[vld])) for vld in xrange(len(allST))]
            x0news = [(allDLx[vld][C[vld]])*B[vld] for vld in xrange(len(allST))]
            y0news = [(allDLy[vld][C[vld]])*B[vld] for vld in xrange(len(allST))]
        ...
        InsModelClass.calculateCG(X, Y, x, y, self.t, x0, y0, self.admST, self.openOcean)
        InsModelClass.mapping(spillname, X, Y, self.admST, self.t)
        ...
        self.TitleEdit.setText("Prediction for Spill %s at Time %.3f Days - Recalc. Area" %(spillname, self.t[0]+ self.admST))

        self.CurrentTimeEdit.setText("%.3f days" % (self.t[0]+ self.admST))

        self.MyLegendLoad(SOSimPath + "/Data/Legend.png", "Legend.png")

        self.canvas.saveAsImage("Spill %s predicted at time %.2f days.png" % (spillname, self.t[0]+ self.admST))
        shutil.copy(SOSimPath + "/Data/Legend.png", ResultsPath + "/Legend.png")

        print "Started recalculation %s" % T1
        print "Finished %s" % time.asctime()
        self.progressBar.setValue(100)
        QMessageBox.information(self, "SOSim - Predictive Maps Ready", "The output maps for the requested prediction time(s) are now on the Results folder of your SOSim directory (C:\Program Files\SOSim\Results).")
...
#__________________________________________________________________________________________________________________________
    ...
    def on_NextTimeButton_clicked(self):
        # spillname = self.spillName()
        # self.evening = self.genForward.next()
        # print "self.evening:", self.evening
        # file = PlayPath + "/Spill %s predicted at time %.1f days.png" % (spillname, self.evening[0])
        # fileName = "Spill %s predicted at time %.1f days" % (spillname, self.evening[0])
        # self.MyRasterLoad(file, fileName)
        # self.rearangeLayers()
        # self.TitleEdit.setText("Prediction for Spill %s at Time %.3f Days" %(spillname, self.evening[0]))
        # self.CurrentTimeEdit.setText("%.3f days" % (self.evening[0]))
        # self.canvas.saveAsImage(ResultsPath + "/Spill %s predicted at time %.3f days.png" % (spillname, self.evening[0]))
        global k_zt
        global cur
    ...
    def on_PreviousTimeButton_clicked(self):
        ...
        # spillname = self.spillName()
        # self.morning = self.genBackward.next()
        # print "self.morning:", self.morning
        # file = PlayPath + "/Spill %s predicted at time %.1f days.png" % (spillname, self.morning[0])
        # fileName = "Spill %s predicted at time %.1f days" % (spillname, self.morning[0])
        # self.MyRasterLoad(file, fileName)
        # self.rearangeLayers()
        # self.TitleEdit.setText("Prediction for Spill %s at Time %.3f Days" %(spillname, self.morning[0]))
        # self.CurrentTimeEdit.setText("%.3f days" % (self.morning[0]))
        # self.canvas.saveAsImage(ResultsPath + "/Spill %s predicted at time %.3f days.png" % (spillname, self.morning[0]))
...
class myDialog(QDialog, Ui_MyDialog):
    def __init__(self, parent = None):
        ...
        self.DxMaxSpinBox.setValue(1.10)
        self.DyMaxSpinBox.setValue(1.10)
        ...
        self.Dxmin = 0.01
        self.Dymin = 0.01
        self.Dxmax = 1.10     # Units in km^2/day
        self.Dymax = 1.10
        self.vxmin = -3.0
        self.vymin = -3.0
        self.vxmax = 3.
