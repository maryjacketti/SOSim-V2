...
def sampleprior(a): # sample data from OSCAR 
    ...
    number = a.N # field data number 
    repeat = 10 
    number = repeat*number    
    n = 10 # dividing into a 10*10 mesh grid 
    number = number*(1./xx)
    a.size = int(number/(n*n))#a.size
    nn = int(number - a.size*n*n) # the number of extra 
    print 'nn',nn
    a.size2 = 1
    scale = [0.27,0.27]
    ...
    #----------------------------------------------------------
    px_decimal = np.array(list(map(lambda a: list(map(lambda x: round(x,8), a)), [a.ppx])))
    py_decimal = np.array(list(map(lambda a: list(map(lambda x:round(x,8), a)), [a.ppy])))
    px_decimal = px_decimal[0]
    py_decimal = py_decimal[0]

    ttotalcon = [] 
    a.priorlon = np.array(list(map(lambda a: list(map(lambda x: round(x,8), a)), [a.fp[:,1]]))) # fp[:,1] is the longituide 
    a.priorlat = np.array(list(map(lambda a: list(map(lambda x:round(x,8), a)), [a.fp[:,0]]))) # fp[:,0] is the latitude 
    a.priorlon=a.priorlon[0]
    a.priorlat=a.priorlat[0]    
    d = {'Lon':a.priorlon,'Lat':a.priorlat,'Con':a.DLclp,'Time':a.ttp}
    df1 = pd.DataFrame(data=d)

    att=(np.unique(a.ttp))
    pxa, pya = np.meshgrid(px_decimal,py_decimal)
    pxa=pxa.flatten()
    pya=pya.flatten()
    tttt = np.array([[att[i]]*len(pxa) for i in range(len(att))])
    tttt = tttt.flatten()

    d2 = {'Lon':list(pxa)*len(att),'Lat':list(pya)*len(att),'Con':[0]*len(pya)*len(att),'Time':tttt}
    df2 = pd.DataFrame(data=d2)

    df=pd.merge(df1,df2,on=['Lon','Lat','Time'],how='right',indicator='Exist')
    df['Exist']=np.where(df.Exist=='both',True,False)
    df['Con_y'][df['Exist']==True]=df['Con_x']  # dataframe with zero
    #---------------------------------------------------------
    px1,py1 = np.meshgrid(a.ppy,a.ppx) 
    px1=px1.flatten()
    py1=py1.flatten()
    ...
    mm1 = []
    for i in range(nn):
        l1 = np.random.choice(range(n),1,replace=False)
        m1 = np.random.choice(range(n),1,replace=False)
        ll1.append(l1[0])
        mm1.append(m1[0]) 

# divide all data into different grids 
    df = df[df['Lat']>=(a.lat0)-scale[0]-0.005]
    df = df[df['Lat']<=(a.lat0)+scale[0]+0.005]
    df = df[df['Lon']>=(a.lon0)-scale[1]-0.005]
    df = df[df['Lon']<=(a.lon0)+scale[1]+0.005]

    df['comp1'] = df['Lon'].apply(lambda row:int((row-a.Y[0])/(2*(scale[0]+0.005)/n)))
    df['comp2'] = df['Lat'].apply(lambda row:int((row-a.X[0])/(2*(scale[0]+0.005)/n)))
    lindex = []
    nindex = []
    dfsample_true=pd.DataFrame(columns=df.keys())#data={'Con_x':[],'Lat':[],'Lon':[],'Time':[],'Con_y':[],'Exist':[],'comp1':[],'comp2':[]})
    for i in range(n):
        for j in range(n):
            if (i,j) in list(zip(ll1,mm1)):
                dfi=df[(df['comp1']==i) & (df['comp2']==j)]
                if len(dfi)>a.size+1:
                    dfsample_true = dfsample_true.append(dfi.sample(n=a.size+1,replace=False),ignore_index=True,sort=False)
                else:
                    lindex.append([i,j])
                    nindex.append(a.size+1-len(dfi))
                    dfsample_true = dfsample_true.append(dfi,ignore_index=True,sort=False)                
            else:
                dfi=df[(df['comp1']==i) & (df['comp2']==j)]
                if len(dfi)>a.size:
                    dfsample_true = dfsample_true.append(dfi.sample(n=a.size,replace=False),ignore_index=True,sort=False)           
                else:
                    lindex.append([i,j])
                    nindex.append(a.size-len(dfi))                
                    dfsample_true = dfsample_true.append(dfi,ignore_index=True,sort=False)    
    lon = dfsample_true['Lon']
    lat = dfsample_true['Lat']
    con = dfsample_true['Con_y']
    priort = dfsample_true['Time']
...
class soscore(Preliminars):
    ...
    def priorandfield(self):  # upload Campaign part
        ...
        ssti = []             
        if len(time)>0: 
            priorlat,priorlon,priorcon,priorx,priory,priort = sampleprior(self) 
    ...
    def writeNetCDF(self):  # record oscar data to a excel file 
        ...
        timein = np.where(self.OSCARt<=np.max(self.st))
        ...
        fp = np.array(fp)   # location 
        DLclp = np.array(DLclp) #concentration 
        ttp = np.array(ttp) # time 

        self.DLclp = DLclp # concentration
    ...
    def upNet(self,File):  # when click Submerged hydrodynamic Upload, OSCAR part 
        ...
        ppx = data.variables['longitude'][:]  # 1 dimension 
        ppy = data.variables['latitude'][:]
        [px,py] = np.meshgrid(ppx,ppy)        
        dep=data.variables['depth'][:]
        ...
        self.px = px
        self.py = py
        self.ppx = ppx # original oscar longitude  
        self.ppy = ppy # original oscar latitude 
        self.index = index
        ...
        if ourinformation['Type'] == 'continuous':
            ...
            for k in range(len(a.DLxx)):
                ...
                if ourinformation['SubmergedType']== "OSCAR" or ourinformation['SubmergedType']== "GNOME" or ourinformation['SubmergedType'] == 'Other':
                    ...
                    if len(Datax) > 0 and len(DDLx) > 0:
                        if np.min(DDLcon) == 0 and np.max(DDLcon) == 0:
                            pass
                        else:
                            DDLcon = DDLcon*(np.max(Datax)/np.max(DDLcon))
                    if len(DDLx)>0 and len(Datax)>0: # Darax is field data # DDLx is prior data 
